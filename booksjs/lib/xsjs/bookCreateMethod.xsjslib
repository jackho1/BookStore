...
function findAuthorByName(authorName, connection) {
	...
	var query = connection.prepareStatement("SELECT * FROM \"BookStore.booksdb::store.Author\" WHERE \"authorName\" LIKE" + "'%" + authorName +
		"%'");
	var queryResult = query.executeQuery();
	query.close();
	while (queryResult.next()) {
...
function findAuthorById(authorId, connection) {
	// TODO: change LIKE to equal 
	var query = connection.prepareStatement("SELECT * FROM \"BookStore.booksdb::store.Author\" WHERE \"authorId\" =" + authorId);
	var queryResult = query.executeQuery();
	query.close();
	while (queryResult.next()) {
		return queryResult;
...
function insertNewAuthor(authorName, connection) {
	var currentAuthorIdQuery = connection.prepareStatement("SELECT \"BookStore.booksdb::authorseq\".NEXTVAL FROM DUMMY");
	...
	var query = connection.prepareStatement("insert into \"BookStore.booksdb::store.Author\" values(?,?,?,?)");
...
function insertNewBook(bookName, authorId,isbn,price, connection) {
	...
	var currentBookIdQuery = connection.prepareStatement("SELECT \"BookStore.booksdb::bookseq\".NEXTVAL FROM DUMMY");
	...
	var query = connection.prepareStatement("INSERT INTO \"BookStore.booksdb::store.Book\" VALUES(?,?,?,?,?,?,?)");
...
function incrementNumberOfBooksForAuthorWithId(authorId, connection) {
	...
	if (authorRecord) {
		...
		var query = connection.prepareStatement("UPDATE \"BookStore.booksdb::store.Author\" SET \"numberOfBooks\" = ? WHERE \"authorId\" = ?");
